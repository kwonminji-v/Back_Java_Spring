...
public class Ex_11_Throws {
	...
	public static void main(String[] args) throws IOException {
				...
				try {
					...
					//writeUTF() 메서드는 문자열 UTF-8의 변형된 형식으로 출력
					out.writeUTF("자신의 생각을 바꾸지 못하는 사람은 결코 현실을 바꿀 수 없다.");
					
					//flush() : 버퍼를 수동으로 비우기, 남아있는 것이 있으면 전부 비운다. / 버퍼에 잔류하는 모든 바이트를 출력한다.
					out.flush();
					...
					//data.bin 객체에 저장된 내용을 저장 형식에 맞춰 읽어들임 이진 파일임
					System.out.println(in.readDouble());
					System.out.println(in.readInt());
					System.out.println(in.readUTF());
				} finally {
					//try-catch문에서 finally는 무조건 실행된다 마지막으로 반드시
					//코드가 정상 작동 되면 finally를 통해 input, output 객체들을 종료 시킨다.
